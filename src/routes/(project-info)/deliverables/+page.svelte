...
<section class="mx-auto max-w-screen-xl px-4 py-8 lg:px-6 lg:py-16">
	<div class="mx-auto mb-12 max-w-screen-sm text-center lg:mb-16">
		<h2 class="mb-4 text-3xl font-bold tracking-tight text-gray-900 dark:text-white lg:text-4xl">
			Case Study / Mini-Project Deliverables
		</h2>
		<p class="font-light text-foreground/80 sm:text-xl">
			The results from this case study are presented below as {data.deliverables.length} separate deliverables.
			Together, they make up the user experience (UX) strategy for this conceptual mobile app.
		</p>
	</div>
		...
		{#each data.deliverables as deliverable, i}
			<Card.Root class="flex min-h-96 flex-col space-y-4 rounded-lg border p-2 shadow-md">
				...
				<Card.Footer class="flex items-center justify-between md:space-x-4">
					...
					<Button size="lg" class="w-3/5" href={`/deliverables/${deliverable.slug}`}>
						Read More
